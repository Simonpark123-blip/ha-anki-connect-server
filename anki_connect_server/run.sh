...
export ANKICONNECT_COLLECTION_PATH="/share/anki/collection.anki2"
...
if [ -f "$ANKICONNECT_COLLECTION_PATH" ]; then
    chmod 666 "$ANKICONNECT_COLLECTION_PATH"
fi

if [ ! -f "$ANKICONNECT_COLLECTION_PATH" ]; then
    echo "[ERROR] Collection not found: $ANKICONNECT_COLLECTION_PATH"
    echo "[ERROR] Create /share/anki and place your initial collection.anki2 there."
    exit 1
fi
echo "[DEBUG] User:"
id
...
ls -l "$ANKICONNECT_COLLECTION_PATH"
...
echo "[INFO] Collection: $ANKICONNECT_COLLECTION_PATH"
